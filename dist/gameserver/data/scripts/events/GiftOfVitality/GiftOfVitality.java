		...
		switch(type)
		{
			case VITALITY:
				...
				else
				{
					...
					player.altOnMagicUse(player, SkillTable.getInstance().getInfo(23179, 1));
					...
					if(servitors.length > 0)
					{
						for(Servitor s : servitors)
						{
							for(int[] buff : _summonBuff)
							{
								...
								player.altOnMagicUse(s, SkillTable.getInstance().getInfo(buff[0], buff[1]));
							}
						}
					}
				...
				else
				{
					if(!player.isMageClass() || player.getTemplate().getRace() == Race.ORC)
						for(int[] buff : _warrBuff)
						{
							...
							player.altOnMagicUse(player, SkillTable.getInstance().getInfo(buff[0], buff[1]));
						}
					else
