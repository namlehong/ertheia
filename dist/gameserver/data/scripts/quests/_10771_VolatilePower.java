	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("inser_fragment"))
		{
			if(st.getCond() == 1)
			{
				...
				MonsterInstance eater1 = st.addSpawn(FRAGMENT_EATER, npc.getLoc().getX() - 40, npc.getLoc().getY(), npc.getLoc().getZ(), 180000);
				MonsterInstance eater2 = st.addSpawn(FRAGMENT_EATER, npc.getLoc().getX(), npc.getLoc().getY(), npc.getLoc().getZ(), 180000);
				MonsterInstance eater3 = st.addSpawn(FRAGMENT_EATER, npc.getLoc().getX() + 40, npc.getLoc().getY(), npc.getLoc().getZ(), 180000);
