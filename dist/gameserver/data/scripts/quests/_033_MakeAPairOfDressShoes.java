...
public class _033_MakeAPairOfDressShoes extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getQuestItemsCount(LEATHER) >= 200 && st.getQuestItemsCount(THREAD) >= 60 && st.getQuestItemsCount(ADENA_ID) >= 500000)
			{
				st.takeItems(LEATHER, 200);
				st.takeItems(THREAD, 60);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30838)
		{
			...
			else if(cond == 3 && st.getQuestItemsCount(LEATHER) >= 200 && st.getQuestItemsCount(THREAD) >= 60 && st.getQuestItemsCount(ADENA_ID) >= 500000)
