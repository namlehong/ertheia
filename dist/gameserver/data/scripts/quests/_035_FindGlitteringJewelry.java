...
{
	//Glory: 5 Орихарукона, 250 Самоцветов для Аксессуаров (34992), 900 Обломков Доспехов (34991)
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equals("30091-3.htm") && cond == 3)
		{
			...
			if(st.getQuestItemsCount(ORIHARUKON) >= 5 && st.getQuestItemsCount(PART_ARM) >= 900 && st.getQuestItemsCount(GEM_FOR_ACC) >= 250)
			{
				st.takeItems(ORIHARUKON, 5);
				st.takeItems(PART_ARM, 900);
				st.takeItems(GEM_FOR_ACC, 250);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30091)
		{
			...
			else if(cond == 4 && (st.getQuestItemsCount(ORIHARUKON) < 5 || st.getQuestItemsCount(PART_ARM) < 900 || st.getQuestItemsCount(GEM_FOR_ACC) < 250))
