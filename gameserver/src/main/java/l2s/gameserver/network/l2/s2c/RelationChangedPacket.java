...
public class RelationChangedPacket extends L2GameServerPacket
{
	...
	protected final List<RelationChangedPacketData> _data;

	protected RelationChangedPacket(int s)
	{
		_data = new ArrayList<RelationChangedPacketData>(s);
	}

	protected void add(RelationChangedPacketData data)
	{
		_data.add(data);
	}
	...
	{
		writeD(_data.size());
		for(RelationChangedPacketData d : _data)
		{
			writeD(d.charObjId);
			writeD(d.relation);
	...
	static class RelationChangedPacketData
	{
		public final int charObjId;
		public final boolean isAutoAttackable;
		public final int relation, karma, pvpFlag;

		public RelationChangedPacketData(Playable cha, boolean _isAutoAttackable, int _relation)
		{
	...
	public static L2GameServerPacket update(Player sendTo, Playable targetPlayable, Player activeChar)
	{
		...
		pkt.add(new RelationChangedPacketData(targetPlayable, targetPlayable.isAutoAttackable(activeChar), relation));
		//if(pet != null)
		//	pkt.add(new RelationChangedPacketData(pet, pet.isAutoAttackable(activeChar), relation));
