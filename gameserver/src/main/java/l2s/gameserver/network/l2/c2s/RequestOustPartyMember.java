...
public class RequestOustPartyMember extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(r != null && r instanceof DimensionalRift && member.getReflection().equals(r))
			activeChar.sendMessage(new CustomMessage("l2s.gameserver.clientpackets.RequestOustPartyMember.CantOustInRift", activeChar));
		else if(r != null && !(r instanceof DimensionalRift))
			activeChar.sendMessage(new CustomMessage("l2s.gameserver.clientpackets.RequestOustPartyMember.CantOustInDungeon", activeChar));
		else
