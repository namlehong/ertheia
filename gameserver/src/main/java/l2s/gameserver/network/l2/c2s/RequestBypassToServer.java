	...
	protected void runImpl()
	{
		...
		try
		{
			...
			else if(bp.bypass.startsWith("player_help "))
			{
				...
				{
					playerHelp(activeChar, helpParams[1], Integer.parseInt(helpParams[2]));
				}
			}
			...
			else if(bp.bypass.startsWith("_olympiad?"))
			{
				...
				if(firstVal.equalsIgnoreCase("move_op_field"))
				{
					...
					if((activeChar.getLastNpc().getNpcId() == 135 && activeChar.getLastNpc().isInRange(activeChar, Creature.INTERACTION_DISTANCE)) || activeChar.getFightBattleObserverArena() != null)
					{
