...
public class RequestActionUse extends L2GameClientPacket
{
		...
		else if(action.type == 1) // Действия петов
		{
			...
			switch(action.id)
			{
				...
				case 16: // Атака петом
					...
					if(!_ctrlPressed && target.isServitor() && activeChar.isMyServitor(target.getObjectId()))
						return;
					...
					if(target.isFakePlayer() && ((Creature) target).isInZonePeace() && !Config.ALLOW_FAKE_PEACE_ZONE_KILL)
					{
		...
		else if(action.type == 2) // Действия саммонов
		{
			...
			switch(action.id)
			{
				...
				case 22: // Атака петом
				{
					...
					if(!_ctrlPressed && target.isCreature() && !((Creature) target).isAutoAttackable(summon))
						return;

					if(_ctrlPressed && !target.isAttackable(summon))
					{
						activeChar.sendPacket(SystemMsg.INVALID_TARGET);
						return;
					}

					if(!target.isMonster() && (summon.isInZonePeace() || target.isCreature() && ((Creature) target).isInZonePeace()))
					{
						activeChar.sendPacket(SystemMsg.YOU_MAY_NOT_ATTACK_THIS_TARGET_IN_A_PEACEFUL_ZONE);
						return;
					}
	...
	private boolean servitorUseSkill(Player player, Servitor servitor, int skillId, int actionId)
	{
		...
		if(skillLevel >= 100)
		{
			EnchantSkillLearn sl = SkillTreeTable.getSkillEnchant(skillId, skillLevel);
			if(sl == null)
				return false;
		...
		Skill skill = SkillTable.getInstance().getInfo(skillId, serverSkillLevel);
		if(skill == null)
			return false;
