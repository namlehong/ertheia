		...
		for(Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
		{
			...
			for(Iterator<Element> skillsIterator = cubicElement.elementIterator(); skillsIterator.hasNext();)
			{
				...
				for(Iterator<Element> skillIterator = skillsElement.elementIterator(); skillIterator.hasNext();)
				{
					...
					int serverSkillLevel = level2;
					if(level2 >= 100)
					{
						EnchantSkillLearn sl = SkillTreeTable.getSkillEnchant(id2, level2);
						if(sl != null)
							serverSkillLevel = SkillTreeTable.convertEnchantLevel(sl.getBaseLevel(), level2, sl.getMaxLevel());
						else
							warn("Wrong skill ID: " + id2 + " lvl. Cubic: " + id + "/" + level);
					}

					Skill skill = SkillTable.getInstance().getInfo(id2, serverSkillLevel);
					if(skill != null)
					{
