...
public class Quest
{
	...
	public Quest(int party)
	{
		...
		if(!Config.USE_QUEST_REWARD_PENALTY_PER || !Config.F2P_QUEST_REWARD_PENALTY_QUESTS.contains(_questId))
			_rewardRate = Config.RATE_QUESTS_REWARD;
		else
			_rewardRate = Math.max(0., Config.RATE_QUESTS_REWARD * ((100. - Config.F2P_QUEST_REWARD_PENALTY_PER) / 100));
	...
	protected final void enterInstance(QuestState st, int instancedZoneId, Object... args)
	{
		...
		if(reflection != null)
		{
			if(player.canReenterInstance(instancedZoneId))
			{
				...
				onReenterInstance(st, reflection, args);
			}
		}
