...
public class Mount
{
	...
	public void onRide()
	{
		switch(getType())
		{
			case STRIDER:
				if(_rider.isNoble())
				{
					_rider.addSkill(SkillTable.getInstance().getInfo(Skill.SKILL_STRIDER_ASSAULT, 1), false);
					_rider.sendPacket(new SkillListPacket(_rider));
				}
				break;
			case WYVERN:
				_rider.setFlying(true);
				_rider.setLoc(_rider.getLoc().changeZ(32));
				_rider.addSkill(SkillTable.getInstance().getInfo(Skill.SKILL_WYVERN_BREATH, 1), false);
				_rider.sendPacket(new SkillListPacket(_rider));
				break;
		}
	...
	public void onUnride()
	{
		...
		_rider.broadcastPacket(new RidePacket(_rider));
		_rider.broadcastUserInfo(true); // нужно послать пакет после Ride для корректного отображения скорости
		_rider.sendPacket(new SetupGaugePacket(_rider, SetupGaugePacket.Colors.GREEN_MINI, 0));
	}
	...
	public void updateStatus()
	{
		...
		_rider.sendPacket(new SetupGaugePacket(_rider, SetupGaugePacket.Colors.GREEN_MINI, time, timeLost));
