...
public class ConditionPlayerCanTransform extends Condition
{
	...
	protected boolean testImpl(Env env)
	{
		...
		if(template.getType() == TransformType.FLYING && (player.getX() > -166168 || player.getZ() <= 0 || player.getZ() >= 6000 || player.getServitors().length > 0 || player.getReflection() != ReflectionManager.DEFAULT))
		{
			player.sendPacket(new SystemMessagePacket(SystemMsg.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addSkillName(skill));
			return false;
		}
		...
		if(player.isTransformImmune() || player.getEffectList().containsEffects(Skill.SKILL_MYSTIC_IMMUNITY))
		{
