...
public class PvPRewardManager
{
	...
	private static boolean basicCheck(Player killed, Player killer)
	{
		if(killed.getLevel() < Config.PVP_REWARD_MIN_PL_LEVEL)
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но уровень противника не подходит. Минимальный уровень: " + Config.PVP_REWARD_MIN_PL_LEVEL);
				else
		...
		if(killed.getClassLevel() < Config.PVP_REWARD_MIN_PL_PROFF)
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но его профессия слишком низкая. Минимальный уровень профессии: " + (Config.PVP_REWARD_MIN_PL_PROFF - 1));
				else
		...
		if((System.currentTimeMillis() - killer.getLastAccess()) < (Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE * 60000))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но вы не достаточно провели время в игре, минимальное время в игре: " + Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE + " мин.");
				else
		...
		if((System.currentTimeMillis() - killed.getLastAccess()) < (Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE * 60000))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но он не достаточно время в игре, минимальное время в игре: " + Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE + " мин.");
				else
		...
		if(!Config.PVP_REWARD_PK_GIVE && killer.isPK())
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но ПК убийства запрещены.");
				else
		...
		if(!Config.PVP_REWARD_ON_EVENT_GIVE && killer.getTeam() != TeamType.NONE)
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но убийства на турнирах не засчитываться.");	
				else
		...
		if(Config.PVP_REWARD_ONLY_BATTLE_ZONE && !killer.isInZone(ZoneType.battle_zone))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но разрешено убивать игроков только на боевых площадках ( не Арены, олимпиада и т.п ).");
				else
		...
		if(!Config.PVP_REWARD_SAME_PARTY_GIVE)
		{
			if(killer.getParty() != null && killer.getParty() == killed.getParty() && (killer.getParty().getCommandChannel() == null || killer.getParty().getCommandChannel() == killed.getParty().getCommandChannel()))
			{
				...
				{
					if(killer.isLangRus())
						killer.sendMessage("Система пвп: Вы убили игрока но вы находитесь в одной партии, что запрещено.");	
					else
		...
		if(!Config.PVP_REWARD_SAME_CLAN_GIVE && killer.getClan() != null && killer.getClan() == killed.getClan())
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но вы находитесь в одной клане , что запрещено.");	
				else
		...
		if(!Config.PVP_REWARD_SAME_ALLY_GIVE && killer.getAllyId() > 0 && killer.getAllyId() == killed.getAllyId())
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но вы находитесь в одном альянсе, что запрещено.");
				else
		...
		if(!Config.PVP_REWARD_SAME_HWID_GIVE && killer.getNetConnection().getHWID().equals(killed.getNetConnection().getHWID()))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но система определила что это ваше другое окно, это запрещено!");	
				else
		...
		if(!Config.PVP_REWARD_SAME_IP_GIVE && killer.getIP().equals(killed.getIP()))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но ваши ИП совпадают, что запрещено.");
				else
		...
		if(Config.PVP_REWARD_SPECIAL_ANTI_TWINK_TIMER && (System.currentTimeMillis() - killed.getCreateTime()) < (Config.PVP_REWARD_HR_NEW_CHAR_BEFORE_GET_ITEM * 60000 * 60))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но чар был создан недавно, что не разрешено системой. Чар должен быть создан не менее: "+Config.PVP_REWARD_HR_NEW_CHAR_BEFORE_GET_ITEM+" часов назад.");	
				else
		...
		if(Config.PVP_REWARD_ONLY_NOBLE_GIVE && !killed.isNoble())
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но он был не ноблесом.");	
				else
		...
		if(Config.PVP_REWARD_CHECK_EQUIP && !checkEquip(killed))
		{
			...
			{
				if(killer.isLangRus())
					killer.sendMessage("Система пвп: Вы убили игрока но его экипировка слишком плоха.");	
				else
	...
	public static boolean checkEquip(Player killed)
	{
	...
	public static void tryGiveReward(Player victim, Player player)
	{
		...
		{
			player.sendMessage("Система пвп: Вы убили игрока этого игрока совсем недавно! Еще не прошло время до возможности повторного убийства.");
			return;
		}
		...
		{
			victim.sendMessage("Система пвп: вас убили!");
			player.sendMessage("Система пвп: Вы убили игрока!");
		}
	}
