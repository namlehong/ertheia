...
public class HealHpCpPercent extends Skill
{
	private final boolean _ignoreHpEff;

	public HealHpCpPercent(StatsSet set)
	{
		super(set);
	...
	public void useSkill(Creature activeChar, List<Creature> targets)
	{
		for(Creature target : targets)
		{
			if(target != null)
			{
				...
				if(target.isPlayer())
				{
					double addToCp = newHp - addToHp;
					if(addToCp > 0)
					{
						target.setCurrentCp(addToCp + target.getCurrentCp());
						target.sendPacket(new SystemMessagePacket(SystemMsg.S1_CP_HAS_BEEN_RESTORED).addInteger(Math.round(addToCp)));
					}
				}
