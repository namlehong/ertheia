	...
	protected void parseSkill(Node n)
	{
		...
		try
		{
			...
			for(int k = 0; k < d.getChildNodes().getLength(); k++)
			{
				...
				enchantType = Integer.parseInt(enchant.getAttributes().getNamedItem("type").getNodeValue()) - 1;
				if(enchantType < 0)
					_log.warn("Invalid enchant type in skill ID[" + skillId + "]!");

				if(enchantLvls == 0)
				{
					if(enchant.getAttributes().getNamedItem("levels") != null)
						enchantLvls = Integer.parseInt(enchant.getAttributes().getNamedItem("levels").getNodeValue());
					else
						enchantLvls = 30;
				}
				...
				if(enchantGroup == -1)
				{
					if(enchant.getAttributes().getNamedItem("group") != null)
						enchantGroup = Integer.parseInt(enchant.getAttributes().getNamedItem("group").getNodeValue());
				}

				String ename = enchant.getAttributes().getNamedItem("name").getNodeValue();
				for(int r = 1; r <= enchantLvls; r++)
				{
					int level = lastLvl + enchantLvls * enchantType + r;
					EnchantSkillLearn e = new EnchantSkillLearn(skillId, 100 * (enchantType + 1) + r, skillName, "+" + r + " " + ename, (r == 1 ? lastLvl : 100 * (enchantType + 1) + r - 1), lastLvl, enchantLvls, enchantGroup);
					...
					displayLevels.put(level, ((enchantType + 1) * 100 + r));
				}
				...
				for(curr = first; curr != null; curr = curr.getNextSibling())
				{
					if("table".equalsIgnoreCase(curr.getNodeName()))
					{
						...
						table = fillTableToSize(table, enchantLvls);
						Object[] fulltable = etables.get(name);
						if(fulltable == null)
							fulltable = new Object[lastLvl + enchantLvls * 9 + 1];
						System.arraycopy(table, 0, fulltable, lastLvl + (enchantType - 1) * enchantLvls, enchantLvls);
						etables.put(name, fulltable);
					}
				}
			}

			lastLvl += enchantLvls * enchantType;
			...
			for(int i = 0; i < lastLvl; i++)
			{
				...
				current.setEnchantLevelCount(enchantLvls);
