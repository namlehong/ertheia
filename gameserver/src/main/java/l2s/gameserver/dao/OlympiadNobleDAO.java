...
public class OlympiadNobleDAO
{
	...
	public static final String OLYMPIAD_GET_HEROS = "SELECT `char_id`, characters.char_name AS char_name FROM `olympiad_nobles` LEFT JOIN characters ON char_id=characters.obj_Id LEFT JOIN character_subclasses ON char_id=character_subclasses.char_obj_id AND character_subclasses.type=? WHERE characters.obj_Id > 0 AND character_subclasses.class_id = ? AND `competitions_done` >= ? AND `competitions_win` > 0 ORDER BY `olympiad_points` DESC, `competitions_win` DESC, `competitions_done` DESC";
	public static final String GET_EACH_CLASS_LEADER = "SELECT characters.char_name AS char_name FROM `olympiad_nobles` LEFT JOIN characters ON char_id=characters.obj_Id LEFT JOIN character_subclasses ON char_id=character_subclasses.char_obj_id AND character_subclasses.type=? WHERE characters.obj_Id > 0 AND character_subclasses.class_id = ? AND `olympiad_points_past_static` != 0 ORDER BY `olympiad_points_past_static` DESC LIMIT 10";
